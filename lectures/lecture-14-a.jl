...
md"""
# Lecture 14a: Gradient boosting

In this notebook, usage of **gradient boosting** algorithm is showcased.
In the lecture we made sense of the following equations:

$F(x) = G_0(x) + \eta G_1(x) + \eta G_2(x) + \dots,$

where $G_m(x)$ are decision trees that aims to match _pseudo_-residues, $\eta$ is a learning rate.

The function $F(x)$ is built in iterations,

```math
\begin{align}
F_0(x) &= G_0(x) \,,\\
F_1(x) &= F_0(x) + \eta G_1(x) \,,\\
F_2(x) &= F_1(x) + \eta G_2(x) \,,\\
\dots
\end{align}
```

The _pseudo_-residues are defined by

$r_{j,m} = -\left[\frac{\partial L(y,f)}{\partial f}\right]_{f = F_{m-1}(x)}$

Within the termination regions (leafs of the decision tree), labeled by $R_{j,m}$, the function $G_{m}(x)$ is approximated by a constant $\gamma_{j,m}$. These value is chosen to minimize loss function,
...
choice of $\gamma$ is trivial. It's just the mean of the leave enries.

When walking towards the targer with a learning rate $\eta$,
one approches it with a geometric progression,

## Convergence

$\eta + (1-\eta) \eta+(1-\eta)^2 \eta+\dots+\eta(1-\eta)^M = 1-(1-\eta)^M$

Condition $(1-\eta)^M \ll 1$ determined the number of iterations
"""

# ╔═╡ f94c30f0-e91d-4ed1-ab71-5f70e4d0ffe0
theme(:boxed)

# ╔═╡ b13d0d6c-1c97-418b-9936-93e484a3e950
md"""
...
Choose which function to approximate, adjust _max depth_, and _# interations_, as well as the _learning rate_.
"""

# ╔═╡ a9d500cb-c919-41e9-9381-0652d4cff090
@bind f_target Select(list_of_target_functions)

# ╔═╡ d83daefa-6497-40ad-a1e8-79115e07a96e
md"""
## Training regression tree
"""

# ╔═╡ 12e8c936-6cbd-4da2-9a26-78b0cf605d5e
const nTraining = 10000
...
# ╠═╡ show_logs = false
bst = xgboost((X, y); num_round, max_depth, η); # , objective = "reg:mse"

# ╔═╡ 39f3558f-e508-463f-8d3d-b92789491ea4
md"""
## Display result
"""
...
# StatQuest lectures
- video-1 ([YouTube](https://www.youtube.com/watch?v=3CC4N4z3GJc))
- video-2 ([YouTube](https://www.youtube.com/watch?v=2xudPOBz-vs))
- video-3 ([YouTube](https://www.youtube.com/watch?v=jxuNLH5dXCs))
- video-4 ([YouTube](https://youtu.be/StWY5QWMXCw?si=r_8oLVW4Ei-9S4Op))

## ROOT TMVA [documentation](https://root.cern.ch/download/doc/tmva/TMVAUsersGuide.pdf)
- Chapter 7.2 Graduent Boosting

## Alex Rogozhnikov blog
...
manifest_format = "2.0"
project_hash = "40bf70824570e3f5ca1544f7f6b6b2c5d4270b33"
